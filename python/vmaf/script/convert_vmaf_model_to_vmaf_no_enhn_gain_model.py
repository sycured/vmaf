...
def convert_vmaf_model_to_vmaf_no_enhn_gain_model(vmaf_model_path, output_vmaf_neg_model_path):
    ...
    vmaf_svm_model_path = f'{vmaf_model_path}.model'
    output_vmaf_neg_svm_model_path = f'{output_vmaf_neg_model_path}.model'
    shutil.copyfile(vmaf_svm_model_path, output_vmaf_neg_svm_model_path)


if sys.version_info[0] != 2:
    print(
        f'warning: running {os.path.basename(__file__)} skipped - must to use py2 to generate the output pickle file.'
    )
else:
